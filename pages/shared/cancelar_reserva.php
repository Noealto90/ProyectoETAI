...
require_once __DIR__ . '/../../config/conexion.php'; // Asegúrate de que el archivo es correcto
...
if (isset($_POST['id'])) {
    ...
    if ($stmt->execute(['id' => $reservaId])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            header('Location: ver_reservas_super_admin.php?mensaje=exito');
        } else {
            // Si no se afectó ninguna fila, significa que la reserva no existe o ya estaba cancelada
            header('Location: ver_reservas_super_admin.php?mensaje=no_encontrada');
        }
    } else {
        // Si hubo un error en la ejecución de la consulta
        header('Location: ver_reservas_super_admin.php?mensaje=error');
    }
} else {
    // Si no se proporcionó ningún ID
    header('Location: ver_reservas_super_admin.php?mensaje=falta_id');
}
